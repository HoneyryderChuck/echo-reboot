...
describe Models::Taggable do
  ...
  describe "validations" do
    it "should validate the context by the given" do
      subject.tao_tags << build(:tao_tag, tao: subject, tag_context_code: "engagement")
  ...
  describe "for an existing tagged taggable" do
    ...
    before do
      tag1 = create(:tag, tag_translations: [build(:tag_translation, language: Language[:en], value: "translation1")] )
      tag2 = create(:tag, tag_translations: [build(:tag_translation, language: Language[:en], value: "translation2")] )
      tag3 = create(:tag, tag_translations: [build(:tag_translation, language: Language[:en], value: "translation3")] )
      subject.tao_tags << create(:tao_tag, tao: subject, tag_context: TagContext[:affection], tag: tag1)
      subject.tao_tags << create(:tao_tag, tao: subject, tag_context: TagContext[:affection], tag: tag2)
      subject.tao_tags << create(:tao_tag, tao: subject, tag_context: TagContext[:affection], tag: tag3)
    end
    ...
    pending "I should be able to set affections"
  end
